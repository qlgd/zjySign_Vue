...
export default {
  ...
  methods: {
    ...
    async getClassStatusFn (courseId) {
      try {
        ...
        const courseObj = this.courseList.find(
          (item) => item.courseOpenId === data.courseId
        )
    ...
    async onChangeDelay () {
      ...
      try {
        ...
        const i = this.courseList.findIndex(
          (obj) => obj.courseOpenId === this.courseItem.courseId
        )
  ...
  watch: {
    'courseItem.status': function (val) {
      val === 1
        ? (this.courseItem.checked = true)
        : (this.courseItem.checked = false)
    }
  }
}
</script>
